...
public class Colosseum {
    ...
    public static Pokemon buildPokemon() {
        ...
        int tempHit = myScan.nextInt();
        if (!(tempHit > 0 && tempHit <= MAX_HIT_POINTS)) {
            while (tempHit <= 0 || tempHit > MAX_HIT_POINTS) {
                System.out.println("Sorry. Hit points must be between 1 and 50:");
                tempHit = myScan.nextInt();
            }
        }
        tempPokemon.hitPoints = tempHit;

        System.out.println("Split fifty points between attack level and defense level");
        System.out.println("Enter your attack level (1-49):");
        tempHit = myScan.nextInt();
        if (!(tempHit > 0 && tempHit <= MAX_HIT_POINTS - 1)) {
            while (tempHit <= 0 || tempHit > MAX_HIT_POINTS - 1) {
                System.out.println("Sorry. The attack level must be between 1 and 49:");
                tempHit = myScan.nextInt();
            }
        }
        tempPokemon.attackLevel = tempHit;

        System.out.println("Enter your defense level (1-" + (tempPokemon.hitPoints - tempPokemon.attackLevel) + "):");
        tempHit = myScan.nextInt();
        if (!(tempHit > 0 && tempHit <= (tempPokemon.hitPoints - tempPokemon.attackLevel))) {
            while (tempHit <= 0 || tempHit > (tempPokemon.hitPoints - tempPokemon.attackLevel)) {
                System.out.println("Sorry. The defense level must be between 1 and " + (tempPokemon.hitPoints - tempPokemon.attackLevel) + ":");
                tempHit = myScan.nextInt();
            }
        }
        tempPokemon.defenseLevel = tempHit;
    ...
    public static void printWhoIsAhead() {
        ...
        if (firstPokemon.hitPoints > secondPokemon.hitPoints) {
            ...
            System.out.println(secondPokemon.name + " " + firstPokemon.name + " are tied!");
        }
